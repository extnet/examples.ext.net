...
namespace Ext.Net.Examples.Pages.samples.gridpanel.selection_models.checkbox_selection
{
    public class IndexModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            this.X().AddScript("App.GridPanel1.getSelectionModel().select(2)");
            this.X().AddScript("App.GridPanel1.getSelectionModel().select(11, true)");
        }

        public IActionResult OnPostButton1_Click(string selectionJson)
        {
            StringBuilder result = new StringBuilder();

            var selectionIds = System.Text.Json.JsonSerializer.Deserialize<int[]>(selectionJson);

            if (selectionIds.Length < 1)
            {
                result.Append("<b>Selection is empty.</b>");
            }
            else
            {
            ...
            this.GetCmp<Label>("Label1").Html = result.ToString();
